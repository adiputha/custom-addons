...
class IouRequest(models.Model):
    ...
    def action_approve_selected(self):
        ...
        self.ensure_one()
        pending_bills = self.bill_ids.filtered(
            lambda b: b.action == "approve" and b.status == "pending"
        )
    ...
    def action_reject_selected(self):
        ...
        self.ensure_one()
        pending_bills = self.bill_ids.filtered(
            lambda b: b.action == "reject" and b.status == "pending"
        )
        bills_without_remarks = pending_bills.filtered(lambda b: not b.remarks)

        if bills_without_remarks:
            raise UserError(
                _("Please provide remarks for the bills you are rejecting.")
            )
    ...
    def action_complete_iou(self):
        ...
        if abs(self.settlement_amount - self.request_amount) > 0.01:
            raise UserError(
                _(
                    "Settlement amount (%.2f) must equal request amount (%.2f) to complete IOU."
                )
    ...
    def _compute_due_date(self):
        default_days = int(
            self.env["ir.config_parameter"].sudo().get_param("iou.due.days", 10)
        )
        for record in self:
    ...
    def action_cancel(self):
        ...
        if self.state in ["draft", "requested"]:
            ...
            raise UserError(
                _("You can only cancel a request in draft or requested state.")
            )
        return True
    ...
    def action_cash_issued(self):
        ...
        if self.state == "requested":
            ...
            raise UserError(
                _("You can only issue cash for requests in the requested state.")
            )
    ...
    def action_complete_request(self):
        ...
        if self.state in ["hod_approved", "float_manager_approved"]:
            ...
            if self.state != "pending_bill_submission":
                raise UserError(
                    _(
                        "You can only complete requests that are in pending bill submission state."
                    )
                )
    ...
    def action_submit_bills(self):
        ...
        if self.state != "pending_bill_submission":
            raise UserError(
                _(
                    "You can only submit bills for requests in pending bill submission state."
                )
            )
        ...
        if abs(total_bill_amount - self.request_amount) > 0.01:
            raise UserError(
                _(
                    "Total bill amount (%.2f) must equal request amount (%.2f) to submit bills."
                )
                % (total_bill_amount, self.request_amount)
            )

        draft_bills.write({"status": "submitted"})

        self.message_post(
            body=_("Bills submitted successfully. Total amount: %.2f")
            % total_bill_amount,
